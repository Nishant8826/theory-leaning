...
│  Congestion ctrl  │ ✅ Adapts to network  │ ❌ None                │
...
Run the UDP server and client code locally. Modify the client to send the message multiple times and see if all arrive.

### Exercise 2: DNS Timing
Observe UDP DNS response times versus TCP by forcing TCP lookups on a domain using `dig +tcp`.
...
> When low latency matters more than reliability: DNS lookups, real-time video/audio (WebRTC), gaming position updates, metrics shipping (StatsD), and QUIC/HTTP/3.
...
> DNS queries are small (< 512 bytes), stateless, and need to be fast. UDP avoids TCP's 3-way handshake (saves 1.5 RTT). If a response is lost, the client retries.

**Q4: What is UDP's maximum packet size?**
> Theoretical: 65,535 bytes. Practical: ~1,400 bytes (Ethernet MTU 1500 - IP header 20 - UDP header 8). Exceeding MTU causes IP fragmentation, which increases loss probability.

**Q5: Can you build a reliable protocol on top of UDP?**
> Yes — that's exactly what QUIC does. You implement sequence numbers, acknowledgments, retransmission, and flow control in your application layer. This gives you TCP's reliability with UDP's flexibility.
