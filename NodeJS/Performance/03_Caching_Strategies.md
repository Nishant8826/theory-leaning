...
## 🧠 Resource Behavior
*   **Bandwidth:** Redis saves DB bandwidth but uses network bandwidth between the App and Redis.
*   **Latency:** Reduces average response time from 100ms (DB) to 5ms (Cache).

---

## 📐 ASCII Diagrams
```text
[ CLIENT ] --(1)--> [ APP ] --(2: GET)--> [ REDIS ] (Hit!)
    |                 |   <---(3: DATA)----+
    |                 |
    + <---(4: 200)----+

MISS SCENARIO:
[ APP ] --(2: GET)--> [ REDIS ] (Miss!)
  |                     |
  +---(3: SELECT)--> [ DB ] --(4: DATA)--> [ APP ] --(5: SET)--> [ REDIS ]
```
...
*   **A:** In Cache-Aside, the app manages the cache manually. In Write-Through, the app writes to the cache, and the cache *automatically* updates the database.

---

## 🧩 Practice Problems
1.  Implement a simple in-memory cache with an LRU policy using a `Map` and a `Doubly Linked List`.
2.  Calculate how much money a company would save in AWS RDS costs if they reduced DB queries by 80% using Redis.
