...
## 🧠 Resource Behavior
*   **Memory:** Primary indicator. Look for a "Sawtooth" pattern that keeps trending upwards.
*   **Latency:** Increases as the GC works harder to find free memory.
...
[ GLOBAL OBJECT ] (Root)
    ...
    [ MyService ]
       |
    [ RequestLogArray ] <--- FORGOT TO CLEAR!
       |
    [ [Obj1], [Obj2], [Obj3] ... [Obj1,000,000] ]
```
...
## 💼 Interview Q&A
*   **Q:** What is a "Detached DOM Tree" equivalent in Node.js?
*   **A:** A "Detached Stream" or "Dangling Listener"—objects that are technically finished but still have a reference in an EventEmitter or global scope.

---

## 🧩 Practice Problems
1.  Write a script with a deliberate memory leak and use the Node.js `heapdump` module to generate a snapshot.
2.  Use `process.memoryUsage()` inside a `setInterval` to plot a graph of memory growth over time.
