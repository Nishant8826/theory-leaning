...
## 🧠 Resource Behavior
*   **CPU:** Threadpool tasks can utilize multiple CPU cores.
*   **Memory:** Each thread in the pool has its own stack memory.
*   **Efficiency:** The threadpool is "lazy"—threads are only created when needed.
...
[ MAIN EVENT LOOP ]
       |
       |--- Is it Network I/O? ----> [ OS KERNEL (epoll/kqueue) ] (No threads used)
       |
       |--- Is it File/Crypto/DNS? -> [ LIBUV THREADPOOL ]
                                      | [T1] [T2] [T3] [T4] | (Blocking work)
```
...
## 💼 Interview Q&A
*   **Q:** Why does `fs.readFile` use the threadpool while `http.get` doesn't?
*   **A:** Because most Operating Systems do not provide a truly asynchronous API for file system access that is portable and efficient, whereas all modern OSs provide non-blocking networking (epoll/kqueue).

---

## 🧩 Practice Problems
1.  Run the crypto test above and observe the timings. Then, set `UV_THREADPOOL_SIZE = 1` and see how it changes.
2.  Look up which specific modules in Node.js core use the Libuv threadpool.
